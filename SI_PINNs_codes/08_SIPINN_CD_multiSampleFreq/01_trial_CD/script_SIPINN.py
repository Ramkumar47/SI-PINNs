...
# V train data is at 93 pnts (8 step size), and cd pred data is at  185 pnts (4 step size)
...
fid_cd = fid_cd.iloc[np.arange(0,fid_cd.shape[0],8)].reset_index(drop=True) #  V samping at 6.3 Hz
fid_nd = fid_nd.iloc[np.arange(0,fid_nd.shape[0],8)].reset_index(drop=True)
