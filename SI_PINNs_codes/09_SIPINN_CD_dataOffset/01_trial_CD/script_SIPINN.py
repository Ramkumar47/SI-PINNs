...
fid_cd = pd.read_csv("../../01_numericalSolution/computed_data.csv") #  <<== for V training
fid_nd = pd.read_csv("../../02_dataProcessing/normalized_data.csv")

# first reducing all to 370 data points
fid_cd_f = fid_cd_f.iloc[np.arange(0,fid_cd_f.shape[0],2)].reset_index(drop=True)
fid_nd_f = fid_nd_f.iloc[np.arange(0,fid_nd_f.shape[0],2)].reset_index(drop=True)
...
# inducing offset by selecting evey other record
fid_cd_f = fid_cd_f.iloc[np.arange(0,fid_cd_f.shape[0],2)].reset_index(drop=True) #  0.0, 0.08 etc..
fid_nd_f = fid_nd_f.iloc[np.arange(0,fid_nd_f.shape[0],2)].reset_index(drop=True)
fid_cd = fid_cd.iloc[np.arange(1,fid_cd.shape[0],2)].reset_index(drop=True) #  0.04, 0.12 etc..
fid_nd = fid_nd.iloc[np.arange(1,fid_nd.shape[0],2)].reset_index(drop=True)
