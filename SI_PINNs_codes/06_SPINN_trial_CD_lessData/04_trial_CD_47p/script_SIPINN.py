...
fid_cd = fid_cd.iloc[np.arange(0,fid_cd.shape[0],16)].reset_index(drop=True)
fid_nd = fid_nd.iloc[np.arange(0,fid_nd.shape[0],16)].reset_index(drop=True)
...
def training_function():
    ...
    res_V,res_coeff = model.predict_f([time,theta,alpha,dele])
...
V_res,coeff_res = model.predict_f([time,theta,alpha,dele])
...
plt.figure()
plt.plot(V_pred,'-b',label = "pred")
plt.plot(V,'-r',label = "actual")
plt.grid()
plt.legend()
plt.title("V")
plt.xlabel("data count")
