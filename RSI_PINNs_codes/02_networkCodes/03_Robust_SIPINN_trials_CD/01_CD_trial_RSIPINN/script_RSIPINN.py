...
plt.figure()
plt.plot(theta_pred,'-b',label = "pred")
plt.plot(theta,'-r',label = "actual")
plt.grid()
plt.legend()
...
plt.figure()
plt.plot(dele_pred,'-b',label = "pred")
plt.plot(dele,'-r',label = "actual")
plt.grid()
plt.legend()
...
fid = pd.DataFrame(np.transpose([V_pred.numpy().flatten(),
                                 alpha_pred.numpy().flatten(),
                                 theta_pred.numpy().flatten(),
                                 dele_pred.numpy().flatten()]),
                   columns = ["V","alpha","theta","dele"])
